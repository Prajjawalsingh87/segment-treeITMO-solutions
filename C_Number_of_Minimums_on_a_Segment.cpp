#include <bits/stdc++.h>
using namespace std;

/**
 * Problem: Number of Minimums on a Segment
 * Source: ITMO Academy Pilot, Segment Tree 1, Step 1, Problem C
 * Description: 
 * Range update: change element at position i to v.
 * Range query: find the minimum and the count of elements equal to the minimum.
 */

typedef long long ll;

struct Node {
    ll min_val;
    ll count;
};

class SegmentTree {
public:
    vector<Node> tree;
    ll n;

    SegmentTree(ll size) {
        n = size;
        tree.resize(4 * n);
    }

    Node combine(const Node& a, const Node& b) {
        if (a.min_val < b.min_val) return a;
        if (a.min_val > b.min_val) return b;
        return {a.min_val, a.count + b.count};
    }

    void build(const vector<ll>& arr, ll idx, ll l, ll r) {
        if (l == r) {
            tree[idx] = {arr[l], 1};
            return;
        }
        ll mid = (l + r) / 2;
        build(arr, 2 * idx, l, mid);
        build(arr, 2 * idx + 1, mid + 1, r);
        tree[idx] = combine(tree[2 * idx], tree[2 * idx + 1]);
    }

    void update(ll idx, ll l, ll r, ll pos, ll val) {
        if (l == r) {
            tree[idx] = {val, 1};
            return;
        }
        ll mid = (l + r) / 2;
        if (pos <= mid) update(2 * idx, l, mid, pos, val);
        else update(2 * idx + 1, mid + 1, r, pos, val);
        tree[idx] = combine(tree[2 * idx], tree[2 * idx + 1]);
    }

    Node query(ll idx, ll l, ll r, ll lq, ll rq) {
        if (r < lq || l > rq) return { (ll)2e18, 0 };
        if (l >= lq && r <= rq) return tree[idx];
        ll mid = (l + r) / 2;
        return combine(query(2 * idx, l, mid, lq, rq), query(2 * idx + 1, mid + 1, r, lq, rq));
    }

    void build(const vector<ll>& arr) { build(arr, 1, 0, n - 1); }
    void update(ll pos, ll val) { update(1, 0, n - 1, pos, val); }
    Node query(ll lq, ll rq) { return query(1, 0, n - 1, lq, rq); }
};

int main() {
    ios::sync_with_stdio(0); cin.tie(0);
    ll n, q;
    if (!(cin >> n >> q)) return 0;
    
    vector<ll> arr(n);
    for (ll i = 0; i < n; i++) cin >> arr[i];

    SegmentTree st(n);
    st.build(arr);

    while (q--) {
        ll op, x, y;
        cin >> op >> x >> y;
        if (op == 1) { // Point update
            st.update(x, y);
        } else { // Range query
            Node res = st.query(x, y - 1);
            cout << res.min_val << " " << res.count << "\n";
        }
    }
    return 0;
}