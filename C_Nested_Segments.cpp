 ...
 * Source: ITMO Academy Pilot, Segment Tree 1, Step 3, Problem C
 * Description: 
 * Given 2n points representing n segments [li, ri].
 * Each segment occurs twice in the input.
 * For each segment, count how many segments are nested within it.
 */
...
int main() {
    ...
    for (ll i = 0; i < 2 * n; i++) {
        ...
        if (l_pos[val] == -1) {
            ...
            // Segment [l_pos[val], i] ends here.
            // Count segments that have already ended and were started after l_pos[val].
            ans[val] = st.query(l_pos[val], i);
            st.update(l_pos[val], 1);
        }
    }
