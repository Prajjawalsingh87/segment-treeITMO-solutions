...
class segmenttree{
   public:
    ...
    void push(ll l,ll r,ll idx){
        ...
        if(l!=r){
            tree[2*idx]=tree[2*idx]|lazy[idx];
            tree[2*idx+1]=tree[2*idx+1]|lazy[idx];
            lazy[2*idx]=lazy[2*idx]|lazy[idx];
    ...
    ll query(ll idx,ll l,ll r,ll lq,ll rq){
        push(l,r,idx);
        if(r<lq || l>rq) return LLONG_MAX;;
        if(l>=lq && r<=rq){
           
            return tree[idx];
        }
        ll mid=(l+r)/2;
        ll left_child=query(2*idx,l,mid,lq,rq);
        ll right_child=query(2*idx+1,mid+1,r,lq,rq);
