...
int main()
{
    ...
    while (t--)
    {
        ...
        vector<ll>a;
        vector<ll>ans(n,0);
        vector<ll>divide_by_6,divide_by_2,divide_by_3;
        for(ll i=0;i<n;i++){
            ll c;
            cin>>c;
            if(c%6==0){
                divide_by_6.push_back(c);
                continue;
            }
            else if(c%2==0){
                divide_by_2.push_back(c);
                continue;
            }
            else if(c%3==0){
                divide_by_3.push_back(c);
                continue;
            }
            else{
                a.push_back(c);
        ...
        ll k=0,l=n-1;
        for(int i=0;i<divide_by_6.size();i++){
            if(i%2==0) ans[k++]=divide_by_6[i];
            else ans[l--]=divide_by_6[i];
        }
        for(int i=0;i<divide_by_2.size();i++){
            ans[k++]=divide_by_2[i];
        }
        for(int i=0;i<divide_by_3.size();i++){
            ans[l--]=divide_by_3[i];
        }
        for(ll i=k;i<=l;i++){
            ans[i]=a[i-k];
