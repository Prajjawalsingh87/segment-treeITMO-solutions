...
class segmenttree{
  ...
  ll n;
    ...
    void push(ll idx,ll l,ll r){
        ...
        ll mid=(l+r)/2;
        if(l!=r){
            tree[2*idx].first=(tree[2*idx].first*1LL*pow(lazy[idx].first,(mid-l+1)))%m;
            tree[2*idx+1].first=(tree[2*idx+1].first*1LL*pow(lazy[idx].first,(r-mid)))%m;
            tree[2*idx].second=(tree[2*idx].second*1LL*lazy[idx].second)%m;
            tree[2*idx+1].second=(tree[2*idx+1].second*1LL*lazy[idx].second)%m;

            lazy[2*idx].first=(lazy[2*idx].first*1LL*lazy[idx].first)%m;
            lazy[2*idx+1].first=(lazy[2*idx+1].first*1LL*lazy[idx].first)%m;
            lazy[2*idx].second=(lazy[2*idx].second*1LL*lazy[idx].second)%m;
            lazy[2*idx+1].second=(lazy[2*idx+1].second*1LL*lazy[idx].second)%m;
    ...
    void build(vector<ll>&arr,ll l,ll r,ll idx){
        if(l==r){
            ...
            tree[idx].second=arr[l];
            return ;
        }
        ll mid=(l+r)/2;
        build(arr,l,mid,2*idx);
        build(arr,mid+1,r,2*idx+1);
        tree[idx].first=(tree[2*idx].first*1LL*tree[2*idx+1].first)%m;
        tree[idx].second=(tree[2*idx].second+tree[2*idx+1].second)%m;
    }
    void update(ll idx,ll l,ll r,ll lq,ll rq,ll val){
        ...
        if(l>=lq && r<=rq){
            ...
            tree[idx].first=(tree[idx].first*1LL*(pow(val,(r-l+1))%m))%m;
            tree[idx].second=(tree[idx].second*1LL*val)%m;
            lazy[idx].first=(lazy[idx].first*1LL*val)%m;
            lazy[idx].second=(lazy[idx].second*1LL*val)%m;
            return ;
        }
        ll mid=(l+r)/2;
